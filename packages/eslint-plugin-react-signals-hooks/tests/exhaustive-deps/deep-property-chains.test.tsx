...
export function TestDeepDependencies({
	insets,
}: TestDeepDependenciesProps): JSX.Element {
...
export function TestPropertyAccessNonSignal({
	...
	// Incorrect: should warn missing 'theme.colors.button.prop' and unnecessary 'theme'
	const btnStyle2 = useMemo(() => {
		return { backgroundColor: theme["colors"]["button"][prop] };
	}, [prop, theme]);

	// Incorrect: should warn missing 'theme.colors.button.prop'
	const btnStyle3 = useMemo(() => {
		return { backgroundColor: theme["colors"]["button"][prop] };
	}, [prop]);
...
export function TestArrayIndexNonSignal({ data }: ListProps): JSX.Element {
	...
	try {
		...
		// Incorrect: should warn missing data.index and 'data.items[data.index].value' and unnecessary 'data'
		const selected2 = useMemo(() => {
			return data.items[data.index]?.value;
		}, [data]);

		// Incorrect: should warn missing data.index and 'data.items[data.index].value'
...
export function TestTupleIndexNonSignal({ data }: TupleProps): JSX.Element {
	...
	try {
		...
		// Incorrect: should warn missing 'data.items[2].value' and unnecessary 'data'
		const selected2 = useMemo(() => {
			return data.items[2].value;
		}, [data]);

		// Incorrect: should warn missing 'data.items[1].value'
		const selected3 = useMemo(() => {

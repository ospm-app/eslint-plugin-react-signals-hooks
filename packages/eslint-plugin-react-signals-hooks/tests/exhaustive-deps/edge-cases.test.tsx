...
export function TestComplexExpressions(): JSX.Element {
  ...
  const filteredItems = useMemo(() => {
    return data.value.items.filter((item) => item.value === data.value.filter.value);
...
export function TestArrayIndices(): JSX.Element {
  ...
  const getRowSum = (rowIndex: number) => {
    // biome-ignore lint/correctness/useExhaustiveDependencies lint/correctness/useHookAtTopLevel: <explanation>
    return useMemo(() => {

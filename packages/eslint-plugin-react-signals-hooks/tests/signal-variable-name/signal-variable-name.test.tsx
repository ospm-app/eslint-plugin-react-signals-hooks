...
export function TestUsePrefix(): JSX.Element {
	...
	const useSomethingSignal = signal(0); // Incorrect: starts with 'use'
...
// Ensure we don't add duplicate accessors when identifier is already used as object of a member
export function TestNoDuplicateAccessor(): JSX.Element {
	useSignals();

	const count = signal(0); // Should be 'countSignal'
	const sum = count.value + 1; // member usage should remain a single .value after rename

	return <div>{sum}</div>;
}

// Top-level (module scope) variable should get .peek() in non-reactive contexts after rename
const moduleCount = signal(0); // Should be 'moduleCountSignal'

// @ts-expect-error
const moduleSum = moduleCount + 1; // non-reactive read should become moduleCountSignal.peek() + 1

...
function isSignalCreation(
	...
	if (
		...
		if (
			signalsNamespaceImports.has(ns) &&
			(prop === "signal" || creatorNames.has(prop))
		) {
			trackOperation(perfKey, PerformanceOperations.signalCreationFound);
			return true;
		}
...
export const noNonSignalWithSignalSuffixRule = ESLintUtils.RuleCreator(
	(name: string): string => {
	...
	meta: {
		...
		messages: {
			...
			suggestRenameWithoutSuffix:
				"Rename '{{ name }}' to '{{ newName }}' (remove suffix)",
			suggestConvertToSignal:
				"Convert '{{ name }}' to a signal using signal() or useSignal()",
		},
	},
	defaultOptions: [
		{
						...
						(option?.validateExported === true ||
							(parentDecl?.parent &&
								parentDecl.parent.type ===
									AST_NODE_TYPES.ExportNamedDeclaration) !== true) &&
												...
												fix(
													...
													if ("name" in node.id) {
														...
														return fixer.replaceText(
															node,
															`const ${node.id.name} = signal(${initText})`,
														);
													}

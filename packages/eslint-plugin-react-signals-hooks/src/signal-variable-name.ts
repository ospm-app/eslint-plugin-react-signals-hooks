		...
		schema: [
			{
				...
				properties: {
					renameOnly: {
						type: "boolean",
						description:
							"When true, only rename identifiers without adding .value/.peek() accessors",
						default: false,
					},
					performance: {
						type: "object",
		...
		return {
			...
			[AST_NODE_TYPES.VariableDeclarator](
				...
				if (!isValidSignalName(node.id.name, suffixRegex)) {
					const messageId =
						kind === "signal" ? "invalidSignalName" : "invalidComputedName";
								...
								try {
									...
									if (variable) {
										for (const reference of variable.references) {
											...
											fixes.push(
												fixer.replaceText(ref, `${fixedName}${accessor}`),
											);
										}
									}

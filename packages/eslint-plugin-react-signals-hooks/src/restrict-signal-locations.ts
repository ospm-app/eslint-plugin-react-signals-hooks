...
export const restrictSignalLocations = ESLintUtils.RuleCreator((name: string): string => {
      ...
      FunctionDeclaration(node: TSESTree.Node): void {
        componentStack.push({
          ...
          isHook:
            ...
            (('id' in node && node.id !== null && 'id' in node && 'name' in node.id) ||
              'name' in node)
              ? // user defined value
                // eslint-disable-next-line security/detect-non-literal-regexp
                new RegExp(option.customHookPattern).test(
                  'id' in node && node.id && 'name' in node.id ? node.id.name : node.type
                )
              : false,
          node,
        });
      ...
      CallExpression(node: TSESTree.CallExpression): void {
        if (
          ...
          ((node: TSESTree.CallExpression) => {
            ...
            if (isComponent) {
              const isComputed =
                node.callee.type === AST_NODE_TYPES.Identifier && node.callee.name === 'computed';

              if (isComputed && option?.allowComputedInComponents === true) {
                return;
              }

              const messageId = isComputed ? 'computedInComponent' : 'signalInComponent';
          ...
          for (const decl of node.declaration.declarations) {
            if (
              ...
              decl.init.callee.type === AST_NODE_TYPES.Identifier
                ? decl.init.callee.name === 'signal' || decl.init.callee.name === 'computed'
                : false) &&

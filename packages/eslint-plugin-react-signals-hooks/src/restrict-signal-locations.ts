...
function isInAllowedDir(filename: string, allowedDirs: Array<string> | undefined): boolean {
    ...
    return {
      ...
      [AST_NODE_TYPES.ExportDefaultDeclaration](node: TSESTree.ExportDefaultDeclaration): void {
        ...
        if (decl.type === AST_NODE_TYPES.VariableDeclaration) {
          ...
          isSignalCreation(decl.callee, signalCreatorLocals, signalNamespaces) &&
          getSeverity('exportedSignal', option) !== 'off'
        ) {
          context.report({ node: decl, messageId: 'exportedSignal' });
        }
          ...
          isHook:
            ...
            ('id' in node && node.id && 'name' in node.id
              ? // eslint-disable-next-line security/detect-non-literal-regexp
                new RegExp(option.customHookPattern).test(node.id.name as string)
              : false),
          node,
        });
      },

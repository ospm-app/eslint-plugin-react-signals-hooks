    ...
    return {
      ...
      FunctionDeclaration(node: TSESTree.Node): void {
        componentStack.push({
          ...
          isHook:
            ('id' in node && node.id !== null && 'id' in node && node.id && 'name' in node.id) ||
            'name' in node
        ...
        if (
          ...
          ((node: TSESTree.CallExpression) => {
            ...
            if (isComponent || isHook) {
              const isComputed =
                node.callee.type === 'Identifier' && node.callee.name === 'computed';
              ...
              context.report({
                node,
                messageId: isComputed ? 'computedInComponent' : 'signalInComponent',
              });
            }
          })(node);
        }

	...
	if ("children" in node && Array.isArray(node.children)) {
		return node.children.some((child: TSESTree.JSXChild): boolean => {
			return (
				typeof child === "object" &&
				"type" in child &&
				containsSignalAccess(child, suffixRegex)
			);
		});
	}
...
function isSignal(
	...
	if ("object" in node.callee) {
		if (node.callee.object.type === AST_NODE_TYPES.Identifier) {
			base = node.callee.object;
		} else if (node.callee.object.type === AST_NODE_TYPES.MemberExpression) {
			base = getBaseIdentifierFromMemberChain(node.callee.object);
		}
	}

	return (
		base !== null &&
		(hasSignalSuffix(base.name, suffixRegex) || signalVariables.has(base.name))
	);
}

function isInSignalWriteContext(
	node: TSESTree.Node,
	suffixRegex: RegExp,
): boolean {
...
function containsSignalAccessTracked(
	node: TSESTree.Node,
	signalVariables: Set<string>,
): boolean {
		...
		schema: [
			{
				...
				properties: {
					suffix: {
						type: "string",
						description:
							"Suffix used to identify signal variables (default: 'Signal')",
						default: "Signal",
					},
					allowInEffects: {
						type: "boolean",
		...
		return {
			...
			[AST_NODE_TYPES.CallExpression](node: TSESTree.CallExpression): void {
				if (
					...
					isInReactiveContext(node, context) &&
					(!isInSignalWriteContext(node, suffixRegex) ||
					...
					node.callee.property.name === "peek" &&
					node.arguments.length === 0 &&
					// resolve base identifier from the callee.object chain
								...
								: [
										{
											...
											fix(fixer: TSESLint.RuleFixer): TSESLint.RuleFix | null {
												if ("object" in node.callee) {
													// Replace the entire call `signal.peek()` with just `signal`
													return fixer.replaceText(
														node,
														context.sourceCode.getText(node.callee.object),
													);

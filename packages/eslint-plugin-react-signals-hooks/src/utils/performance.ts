...
export function startTracking<Options extends Array<unknown>>(
	...
	if (typeof budget !== "undefined") {
		...
		if (!validation.valid) {
			...
			console.warn(
				`[${perfKey}] Invalid performance options:`,
...
export function stopTracking(key: string): PerformanceMetrics | undefined {
	...
	if (metrics.perfBudget?.logMetrics === true) {
		console.info(
			`[Performance Metrics] ${metrics.ruleName} (${metrics.filePath})`,
		);
...
export function logMetrics<Options extends Array<unknown>>(
...
export function createPerformanceTracker<Options extends Array<unknown>>(
	key: string,
	budget: PerformanceBudget | undefined,
	context: RuleContext<string, Options>,
	...
	return {
		trackNode(node: TSESTree.Node) {
			...
			if (
				...
				currentMetrics.budgetExceededBy =
					(currentMetrics.budgetExceededBy ?? 0) + 1;

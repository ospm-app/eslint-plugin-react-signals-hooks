...
export const preferSignalReadsRule = ESLintUtils.RuleCreator(
	(name: string): string => {
	...
	meta: {
		...
		messages: {
			useValueInNonJSX:
				"Use .value to read the current value of the signal in non-JSX context",
		},
		schema: [
			{
				type: "object",
	...
	create(
		...
		return {
			...
			[AST_NODE_TYPES.Identifier](node: TSESTree.Node): void {
				...
				const isSignalIdent =
					hasSignalSuffix(node.name, suffixRegex) ||
				...
				if (
					p.type === AST_NODE_TYPES.CallExpression &&
			...
			[AST_NODE_TYPES.VariableDeclarator](
				...
				if (callee.type === AST_NODE_TYPES.Identifier) {
					if (
						signalCreatorLocals.has(callee.name) ||
						computedCreatorLocals.has(callee.name)
					) {
						isCreator = true;
					}
				} else if (

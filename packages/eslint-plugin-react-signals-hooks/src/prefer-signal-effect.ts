...
function hasCleanupReturn(
	...
	if (
		...
		return cb.body.body.some(
			(s: TSESTree.Statement): s is TSESTree.ReturnStatement => {
					...
					!(
						...
						node.arguments[1].elements.every(
							(
								...
								return isSignalDependency(
									dep,
									buildSuffixRegex(
										typeof option?.suffix === "string" &&
											option.suffix.length > 0
											? option.suffix
											: "Signal",
									),
								);
				...
				context.report({
					...
					fix(fixer: TSESLint.RuleFixer): Array<TSESLint.RuleFix> | null {
						...
						fixes.push(
							fixer.replaceText(
								node,
								`effect(${context.sourceCode.getText(cb)})`,
							),
						);
						...
						if (!hasEffectImport) {
							...
							const effectImport = buildNamedImport(
					...
					suggest: [
						{
							...
							fix(fixer: TSESLint.RuleFixer): Array<TSESLint.RuleFix> | null {
								...
								fixes.push(
									fixer.replaceText(
										node,
										`effect(${(cb ? context.sourceCode.getText(cb) : "") || "() => { /* TODO: port body */ }"})`,
									),
								);

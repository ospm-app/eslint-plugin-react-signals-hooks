...
function containsSignalUpdate(
	...
	for (const key of Object.keys(node)) {
		...
		if (Array.isArray(value)) {
			...
			typeof value === "object" &&
			containsSignalUpdate(value as TSESTree.Node, context)
		) {
			return true;
		}
	}
...
function processBlock(
	...
	if (
		...
		const unsafeSeparation =
			...
			!isSafeAutofixRange(
				context,
				firstUpdateNode.range[1],
				lastUpdateNode.range[0],
			);
				...
				suggest: [
					{
						...
						*fix(
							...
							const lastUpdate =
								updatesInScope[updatesInScope.length - 1]?.node;
...
function isSignalType(symbol: ts.Symbol, checker: ts.TypeChecker): boolean {
	...
	if (
		...
		type.symbol &&
		["Signal", "ReadableSignal", "WritableSignal"].includes(
	...
	for (const d of decls) {
		if (ts.isVariableDeclaration(d)) {
			...
			if (typeof init !== "undefined" && ts.isCallExpression(init)) {
				...
				if (
					info !== null &&
					info.module.includes("@preact/signals-react") === true
				) {
...
export const preferBatchUpdatesRule = ESLintUtils.RuleCreator(
	...
	create(
		...
		return {
					...
					if (
						...
						if (
							...
							if (Array.isArray(node.arguments[0].body.body)) {
								...
								const updateStmts: Array<TSESTree.ExpressionStatement> =
									bodyStatements
										.filter(
											(
												s: TSESTree.Statement,
											): s is TSESTree.ExpressionStatement => {
								...
								if (
									...
									getSeverity("removeUnnecessaryBatch", context.options[0]) !==
										"off"
								) {
									if (
										// eslint-disable-next-line @typescript-eslint/strict-boolean-expressions, @typescript-eslint/no-unnecessary-condition
							...
							if (
								...
								getSeverity("nonUpdateSignalInBatch", context.options[0]) !==
									"off"
							) {
								context.report({
			...
			[`${AST_NODE_TYPES.AssignmentExpression}:exit`](

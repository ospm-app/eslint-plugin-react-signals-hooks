...
// Gated debug logger for exhaustive-deps debugging
...
function collectRecommendations({
  ...
  console.info(
    '[react-signals-hooks/exhaustive-deps][debug] dependencies',
    JSON.stringify(depDump, null, 2)
  );
  ...
  console.info(
    '[react-signals-hooks/exhaustive-deps][debug] depTree.children',
    JSON.stringify(treeDump, null, 2)
  );
  ...
  function scanTreeRecursively(
    ...
    node.children.forEach((child: DependencyTreeNode, key: string): void => {
      ...
      if (child.isSatisfiedRecursively) {
        ...
        // If this path is already satisfied, do NOT mark it missing.
        // Only short-circuit for signal-related paths; for non-signal bases,
        // continue recursion so we can detect missing deep properties.
        if (isAnySignalType) {
          console.info('short-circuit for signal-related paths');
          return;
        }

        console.info('continue recursion for non-signal bases');
      } else if (
        child.isUsed ||
        ...
        console.info('[react-signals-hooks/exhaustive-deps][debug] add-missing', {
          path,
          isAnySignalType,
          isUsed: child.isUsed,
          isSubtreeUsed: child.isSubtreeUsed,
          isSatisfiedRecursively: child.isSatisfiedRecursively,
          children: Array.from(child.children.keys()),
        });
  ...
  if (baseMissing.length > 0) {
    ...
    for (const base of baseMissing) {
      const deepMissing = Array.from(missingDependencies).filter((m: string): boolean => {
        return m.startsWith(`${base}.`);
      });

      const deepDeclared = declaredDependencies
        .map(({ key }) => {
          return key;
        })
      ...
      console.info(
        '[react-signals-hooks/exhaustive-deps][debug]',
        JSON.stringify(
          {
            perfKey,
            isEffect,
            base,
            deepMissing: deepMissing.sort(),
            deepDeclared: deepDeclared.sort(),
            deepReads: deepReads.sort(),
            leafUsage,
    ...
    for (const s of suggestedDependencies) {
      if (
        ...
        if (deepMissingDependencies.length === 0) {
          dependencies.forEach((depInfo: Dependency, depKey: string): void => {
            if (depKey.startsWith(`${s}.`) && depInfo.hasReads === true) {
...
function visitFunctionWithDependencies(
  ...
  function gatherDependenciesRecursively(currentScope: Scope, pureScopes: Set<Scope>): void {
    ...
    if (typeof objectPropertyAccessesSet !== 'undefined') {
      ...
      // Promote non-signal identifiers to their outermost member expression to capture deep reads
      if (
        ...
        if (parts.length > 2 && parts[1] === 'value') {
          ...
          if (
            ...
            if (isBaseNonSignal) {
              ...
              const hasMissingDeeper = Array.from(missingDependencies).some((d) =>
                d.startsWith(`${key}.`)
              );
              if (hasDeclaredDeeper || hasMissingDeeper) {
                return;
  ...
  if (unnecessaryDependencies.size > 0) {
    ...
    unnecessaryDepsList.forEach((depKey: string): void => {
      ...
      if (!isFirst && depTokenIndex > 0) {
        ...
        if (typeof prevTokenEnd === 'number' && /^\s+$/.test(whitespaceBefore)) {
  ...
  if (duplicateDependencies.size > 0) {
    ...
    if (duplicates.length > 0) {
      ...
      // Report the summary
      const messageId = isSingleDuplicate ? 'duplicateDependency' : 'duplicateDependencies';
  ...
  create(context: Readonly<RuleContext<MessageIds, Options>>, [option]): ESLintUtils.RuleListener {
    ...
    return {
      ...
      CallExpression(node: TSESTree.CallExpression): void {
        ...
        if (!maybeNode && isEffect && option?.requireExplicitEffectDeps === true) {
          const messageId = 'missingEffectCallback';

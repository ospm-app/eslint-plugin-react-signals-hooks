...
function collectRecommendations({
		...
		if (hasValueAccess) {
			const isValueDeclared = declaredDependencies.some(
				({ key }: DeclaredDependency): boolean => {
					return (
						key === valueAccessKey ||
						(key.startsWith(`${valueAccessKey}[`) && key.includes("Signal"))
					);
				},
			);
	...
	function scanTreeRecursively(
		...
		node.children.forEach((child: DependencyTreeNode, key: string): void => {
			...
			const isAnySignalType =
				isSignalPath || isImportedSignal || hasValueAccess;
			...
			if (child.isUsed || (isAnySignalType && child.isSubtreeUsed)) {
...
function markNode(
	...
	try {
		...
		if (optionalChains) {
			if ("optional" in node && typeof node.optional !== "undefined") {
      ...
      objectPropertyAccessesSet.add(fullPath);
			...
			if (
				...
				for (const ref of currentScope.references) {
					if (ref.identifier.name === reference.identifier.name) {
						...
						if (ref.isComputedAssignmentOnly !== true) {
					...
					if (
						...
						if (referenceNode.parent.parent.computed) {
							...
							if (propertyNode.type === "Identifier") {
								...
								propertyNode.expression.type === "Identifier"
							) {
			...
			if (dependencies.has(dependency)) {
				...
				const isImportedSignal =
					...
					(dependency.includes(".") &&
						dependency.split(".")[0]?.endsWith("Signal"));

				const isSignalValueAccessBool = dependency.endsWith(".value");

				if (isImportedSignal === true || isSignalValueAccessBool === true) {
					if (isImportedSignal === true && isSignalValueAccessBool === false) {
						const toDelete = dependency.includes(".")
							? dependency.split(".")[0]
							: dependency;
...
export const exhaustiveDepsRule = ESLintUtils.RuleCreator(
	...
	meta: {
		...
		messages: {
			...
			performanceLimitExceeded:
				"Performance limit exceeded for rule exhaustive-deps {{message}}",
		},
		fixable: "code",
		hasSuggestions: true,
		schema: [

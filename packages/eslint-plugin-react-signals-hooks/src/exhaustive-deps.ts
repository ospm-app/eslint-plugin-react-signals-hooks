...
function collectRecommendations({
	...
	function scanTreeRecursively(
		...
		node.children.forEach((child: DependencyTreeNode, key: string): void => {
			...
			if (
				child.isUsed ||
				(isAnySignalType && (child.isSubtreeUsed || child.isUsed))
			) {
...
function visitFunctionWithDependencies(
			...
			if (
				...
				if (
					...
					if (
						...
						if (
							referenceNode.parent.parent.computed &&
							referenceNode.parent.parent.property
						) {
	...
	if (unnecessaryDependencies.size > 0) {
		...
		unnecessaryDepsList.forEach((depKey: string): void => {
			...
			const depTokenIndex = tokens.findIndex(
				(t) => t.range[0] === depNode.range[0],
			);
	...
	if (duplicateDependencies.size > 0) {
		...
		const normalizedMap = new Map<
			string,
			{ original: string; node: TSESTree.Node }[]
		>();

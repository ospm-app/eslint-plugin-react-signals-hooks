...
function analyzePropertyChain(
  ...
  try {
    ...
    if (error instanceof PerformanceLimitExceededError) {
      ...
      trackOperation(perfKey, 'analyze_property_chain_failed');
...
function collectRecommendations({
  ...
  function scanTreeRecursively(
    ...
    node.children.forEach((child: DependencyTreeNode, key: string): void => {
      ...
      const isSignalPath =
        isSignalDependency(path, perfKey, metrics) || isSignalDependency(key, perfKey, metrics);
...
function scanForConstructions({
  ...
  const constructions = declaredDependencies
    .map(({ key }: DeclaredDependency): (string | Variable)[] | null => {
...
function getDependency(
  ...
  try {
    ...
    if (node.type === 'Identifier' && isSignalVariable(node, perfKey, metrics)) {
      return node;
    }
    ...
    if (node.type === 'JSXExpressionContainer') {
      return getDependency(node.expression, optionalChains, perfKey, metrics);
    }

    if (node.type === 'CallExpression') {
      return getDependency(node.callee, optionalChains, perfKey, metrics);
    }
    ...
    if (error instanceof PerformanceLimitExceededError) {
      metrics.performanceLimitExceeded++;
      trackOperation(perfKey, 'get_dependency_failed');
...
function markNode(
  ...
  try {
    ...
    if (error instanceof PerformanceLimitExceededError) {
      metrics.performanceLimitExceeded++;
      trackOperation(perfKey, 'mark_node_failed');
...
export const exhaustiveDepsRule = createRule<Options, MessageIds>({
  ...
  create(context, [rawFirstOption = {}]): ESLintUtils.RuleListener {
    const perfKey = 'exhaustive-deps';
    ...
    function visitFunctionWithDependencies(
      ...
      function isStableKnownHookValue(resolved: Variable, componentScope: Scope | null): boolean {
        ...
        if (
          (isSignalIdentifier(callee, perfKey, metrics) ||
            isSignalVariable(definitionNode.id, perfKey, metrics)) &&
        ...
        if (
          isUseEffectEventIdentifier(callee, perfKey, metrics) &&
          definitionNode.id.type === 'Identifier'
        ) {
          for (const ref of resolved.references) {
            if (ref.identifier !== definitionNode.id) {
      ...
      function gatherDependenciesRecursively(currentScope: Scope, pureScopes: Set<Scope>): void {
        ...
        for (const reference of currentScope.references) {
          ...
          let dependencyNode = getDependency(referenceNode, optionalChains, perfKey, metrics);
          ...
          if (dependencies.has(dependency)) {
            ...
            dependencies.set(dependency, {
              ...
              isStable:
                ...
                isSignalDependency(dependency, perfKey, metrics) ||
                isImportedSignal ||
    ...
    try {
      ...
      const perf = createPerformanceTracker(
        perfKey,
        {
          ...
          maxTime: performanceConfig.maxTime ?? 50, // ms
          maxNodes: performanceConfig.maxNodes ?? 2000,
          maxMemory: performanceConfig.maxMemory ?? 50 * 1024 * 1024, // 50MB

          // Operation-specific limits
          maxOperations: {
            [PerformanceOperations.hookCheck]: performanceConfig.maxOperations?.hookCheck ?? 1000,
            [PerformanceOperations.effectCheck]:
              performanceConfig.maxOperations?.effectCheck ?? 1000,
            [PerformanceOperations.signalCheck]:
              performanceConfig.maxOperations?.signalCheck ?? 500,
            [PerformanceOperations.nodeProcessing]:
              performanceConfig.maxOperations?.nodeProcessing ?? 10000,
          },

          // Feature toggles
          enableMetrics: performanceConfig.enableMetrics ?? false,
          logMetrics: performanceConfig.logMetrics ?? false,
        },
        context
      );
      ...
      recordMetric(perfKey, 'config', {
        additionalHooks: additionalHooks ? 'custom' : 'default',
        experimental_autoDependenciesHooks: experimental_autoDependenciesHooks.length,
        requireExplicitEffectDeps,
        enableAutoFixForMemoAndCallback,
      });
      ...
      try {
        ...
        let nodeCount = 0;
        const maxNodeCount = option.performance?.maxNodes ?? 2000;

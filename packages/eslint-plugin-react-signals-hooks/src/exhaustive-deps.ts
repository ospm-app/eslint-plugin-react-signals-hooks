...
function getObservedFormatted(
	...
	if (
		...
		dep.observedFormatted &&
		dep.observedFormatted.size > 0
	) {
...
function isSignalVariable(
	node: TSESTree.Node | Pattern,
	perfKey: string,
): boolean {
	trackOperation(perfKey, PerformanceOperations.signalCheck);
	...
	const suffixRegex =
		suffixByPerfKey.get(perfKey) ?? buildSuffixRegex("Signal");
...
function isSignalValueAccess(
	...
	if (
		...
		hasSignalSuffix(
			node.object.name,
			buildSuffixRegex(
				typeof context.options[0]?.suffix === "string" &&
					context.options[0].suffix.length > 0
...
function isAncestorNodeOf(
	...
	return (
		/* !!a.range && !!b.range && */ a.range[0] <= b.range[0] &&
		a.range[1] >= b.range[1]
	);
}
	...
	try {
		...
		if (node.type === AST_NODE_TYPES.MemberExpression) {
			if (!node.computed) {
				...
				const objectPath = analyzePropertyChain(
					node.object,
					optionalChains,
					context,
					perfKey,
			...
			const object = analyzePropertyChain(
				node.object,
				optionalChains,
				context,
				perfKey,
...
function scanTreeRecursively(
	...
	node.children.forEach((child: DependencyTreeNode, key: string): void => {
		...
		const isSignalPath =
			isSignalDependency(path, perfKey) || isSignalDependency(key, perfKey);
...
function collectRecommendations({
	...
	dependencies.forEach(
		({ isStable, references }: Dependency, key: string): void => {
			if (isStable) {
	...
	for (const { key } of declaredDependencies) {
		getOrCreateNodeByPath(
			depTree,
			key.replace(/\?\./g, "."),
		).isSatisfiedRecursively = true;
	}
	...
	importedSignals.forEach((signal: string): void => {
		if (signal.includes(".value[")) {
			...
			if (!isComputedPropertyDeclared) {
				...
				if (valueIndex !== -1) {
					...
					if (signal.includes(".", valueIndex + 7)) {
						const baseSignalSatisfied = getOrCreateNodeByPath(
							depTree,
							signalName,
						);
			...
			const isValueDeclared = declaredDependencies.some(
		...
		if (!isDeclared) {
			...
			if (isSignalDependency(signal, perfKey)) {
				const deepPropertyChains = Array.from(dependencies.keys()).filter(
					(key: string): boolean => {
						return (
							key.startsWith(`${signal}.value[`) && key.includes(".neighbors.")
						);
					},
				);
				...
				const computedMembers = Array.from(dependencies.keys()).filter(
					(key: string): boolean => {
						return key.startsWith(`${signal}.value[`);
					},
				);

				hasAssignmentOnlyComputedMembers = computedMembers.every(
					(key: string): boolean => {
	...
	dependencies.forEach((depInfo: Dependency, index: string): void => {
		if (
			...
			isSignalDependency(index.split(".")[0] ?? "", perfKey)
		) {
	...
	for (const base of Array.from(missingDependencies)) {
		if (!base.includes(".") && !isSignalDependency(base, perfKey)) {
			const hasDeeperMissing = Array.from(missingDependencies).some(
				(d: string): boolean => {
	...
	for (const { key } of declaredDependencies) {
		if (!isSignalDependency(key, perfKey) && key.includes(".")) {
			...
			const isUsed =
				(typeof depInfo !== "undefined" && depInfo.hasReads) ||
				node.isUsed ||
				node.isSubtreeUsed;
	...
	declaredDependencies.forEach(({ key }: { key: string }): void => {
		...
		if (
			key.includes(".value[") &&
			isSignalDependency(key.split(".")[0] ?? "", perfKey)
		) {
			const valueIndex = key.indexOf(".value[");
			...
			if (valueIndex !== -1) {
				...
				if (closingBracket !== -1) {
					...
					if (
						declaredDepsMap.has(baseComputedExpression) &&
						baseComputedExpression !== key
					) {
						const isDeepPropertyChain =
							key.length > baseComputedExpression.length &&
							key.charAt(closingBracket + 1) === ".";

						if (isDeepPropertyChain) {
							const hasOtherDeepPropertyChains = Array.from(
								declaredDepsMap.keys(),
							).some((declaredKey: string): boolean => {
								return (
							...
							if (hasOtherDeepPropertyChains) {
								...
								const isDirectlyUsed =
									baseDependency && baseDependency.hasReads === true;
		...
		if (!key.includes(".") && !isSignalDependency(key, perfKey)) {
			const hasDeepPropertyUsage = Array.from(missingDependencies).some(
				(dep: string): boolean => {
			...
			if (hasDeepPropertyUsage) {
				...
				if (
					(typeof baseDependency !== "undefined" &&
						baseDependency.hasReads === true) !== true
				) {
					incompleteDependencies.add(key);
				...
				Array.from(declaredDepsMap.keys()).some(
	...
	declaredDependencies.forEach(({ key }: { key: string }): void => {
		...
		const isSignalDep =
			isSignalDependency(key, perfKey) || key.endsWith(".value");

		const dependency = dependencies.get(key);

		const isAssignmentOnly =
			isSignalDep && dependency && dependency.hasReads === false;

		if (isAssignmentOnly === true) {
			unnecessaryDependencies.add(key);
		} else if (
			incompleteDependencies.has(key) ||
			redundantDependencies.has(key)
		) {
			unnecessaryDependencies.add(key);
		} else if (
			(isEffect &&
	...
	const baseMissing = Array.from(missingDependencies).filter(
		(m: string): boolean => {
	...
	for (const key of missingDepsArray) {
		...
		if (
			...
			const hasExistingDeepMissing = missingDepsArray.some(
				(m: string): boolean => {
	...
	// Normalize: replace non-signal base suggestions with deep properties if present
	{
		...
		for (const s of suggestedDependencies) {
			if (
				...
				const replacements =
					deepMissingDependencies.length > 0
						? deepMissingDependencies
						: deepReads;
...
function getConstructionExpressionType(
...
function getDependency(
	...
	try {
		...
		if (
			node.type === AST_NODE_TYPES.Identifier &&
			isSignalDependency(node.name, perfKey)
		) {
			return node;
		}
...
function getReactiveHookCallbackIndex(
	...
	switch (node.name) {
		...
		default: {
			if (
				node === calleeNode &&
				typeof options.additionalHooks !== "undefined"
			) {
...
const signalVariables = new WeakSet<
	TSESTree.Identifier | TSESTree.JSXIdentifier
>();
...
function visitFunctionWithDependencies(
			...
			context.report({
				...
				fix(fixer: TSESLint.RuleFixer): TSESLint.RuleFix | null {
					return fixer.replaceText(
						node,
						`async () => { ${context.sourceCode.getText(node)}; }`,
					);
				},
			});
		}
	...
	function isStableKnownHookValue(
		...
		if (
			(isSignalIdentifier(callee, perfKey) ||
				isSignalVariable(definitionNode.id, perfKey)) &&
		...
		if (
			isUseEffectEventIdentifier(callee, perfKey) &&
			definitionNode.id.type === "Identifier"
		) {
			for (const ref of resolved.references) {
				if (ref.identifier !== definitionNode.id) {
	...
	function isFunctionWithoutCapturedValues(
		...
		for (const ref of fnScope.through) {
			...
			if (
				...
				!memoizedIsStableKnownHookValue(
					ref.resolved,
					componentScope,
					pureScopes,
				)
    ...
    if (typeof objectPropertyAccessesSet !== 'undefined') {
      objectPropertyAccessesSet.add(fullPath);
			...
			let dependencyNode = getDependency(
				referenceNode,
				optionalChains,
				perfKey,
			);
			...
			if (
				...
				let outermost: TSESTree.Node | TSESTree.MemberExpression =
					dependencyNode.parent;
					...
					if (referenceNode.parent.parent.computed) {
						...
						if (typeof propertyName === "string" && propertyName !== "") {
							...
							if (propertyRef?.resolved) {
								...
								if (baseValueDep) {
									baseValueDep.hasInnerScopeComputedProperty =
										isInnerScopeProperty;
								}
							}
						}
					}
			...
			if (typeof depKey === "string") {
				...
				const isRefNameHeuristic = /(?:Ref|ref)$/.test(
				...
				if (
					isRefLike &&
					(isCurrentAccess || depKey === reference.identifier.name)
				) {
					continue;
				}
			}
			...
			function isRefTypeAnnotated(definition: Definition | undefined): boolean {
				...
				if (definition.type === "Variable") {
					...
					const typeNode =
						id.type === AST_NODE_TYPES.Identifier ? id.typeAnnotation : null;
			...
			if (dependencies.has(depKey)) {
				...
				if (
					!isOnlyAssignmentReference(reference) &&
					!isComputedAssignmentOnly
				) {
					const dep = dependencies.get(depKey);
				...
				const isImportedSignal =
					...
					(isSignalDependency(depKey, perfKey) ||
						(depKey.includes(".") &&
							isSignalDependency(depKey.split(".")[0] ?? "", perfKey)));

				const isSignalValueAccessBool = isSignalValueAccess(
					reference.identifier,
					context,
				);
				...
				dependencies.set(depKey, {
					...
					hasReads:
						!isOnlyAssignmentReference(reference) && !isComputedAssignmentOnly,
	...
	currentRefsInEffectCleanup.forEach(({ reference, dependencyNode }): void => {
		...
		for (const ref of references) {
			if (
				...
				(!("computed" in ref.identifier.parent) ||
					!ref.identifier.parent.computed) &&
		...
		context.report({
			...
			node:
				"property" in dependencyNode.parent
					? dependencyNode.parent.property
					: dependencyNode,
		});
	});
								...
								fix(fixer: TSESLint.RuleFixer): TSESLint.RuleFix | null {
									const lastArg =
										"arguments" in reactiveHook
											? reactiveHook.arguments[
													reactiveHook.arguments.length - 1
												]
											: null;
		...
		if ("elements" in arrayExpression) {
			arrayExpression.elements.forEach((declaredDependencyNode): void => {
				...
				if (declaredDependencyNode.type === AST_NODE_TYPES.SpreadElement) {
					...
					const spreadSource = context.sourceCode.getText(
						...
						suggest: [
							{
								...
								fix(fixer: TSESLint.RuleFixer): TSESLint.RuleFix | null {
									...
									const prevToken = context.sourceCode.getTokenBefore(
				...
				try {
					...
					if (
						...
						(declaredDependencyNode.type === AST_NODE_TYPES.MemberExpression &&
							declaredDependencyNode.property.type ===
								AST_NODE_TYPES.Identifier &&
							declaredDependencyNode.property.name === "bind")
					) {
						let message = "";
						let suggestion: SuggestionReportDescriptor<MessageIds> | null =
							null;
						...
						if (declaredDependencyNode.type === AST_NODE_TYPES.CallExpression) {
							...
							suggestion = {
								...
								fix(fixer: TSESLint.RuleFixer): Array<TSESLint.RuleFix> | null {
									...
									if (effectBody.type !== AST_NODE_TYPES.BlockStatement) {
										return null;
									}
				...
				try {
					...
					if (
						...
						if (declaredDependencyNode.type === AST_NODE_TYPES.Literal) {
							...
							const nodeText = context.sourceCode.getText(
								declaredDependencyNode,
							);
					...
					maybeID =
						"object" in maybeID
							...
							: "expression" in maybeID
								? typeof maybeID.expression === "object" &&
									"object" in maybeID.expression
	...
	if (unnecessaryDepsList.length > 0) {
		...
		const externalDep =
			externalDependencies.size > 0
				? Array.from(externalDependencies)[0]
				: null;
					...
					if (!isComputedPropertyDeclared) {
						...
						if (!isAssignmentOnly) {
							...
							if (!(isInnerScopeComputed && isBaseValueDeclared)) {
								...
								if (isBaseNonSignal) {
									const hasDeclaredDeeper = declaredDependencies.some(
										({ key: dKey }: DeclaredDependency): boolean => {
											return dKey.startsWith(`${key}.`);
										},
									);
									const hasMissingDeeper = Array.from(missingDependencies).some(
										(d: string): boolean => {
											return d.startsWith(`${key}.`);
										},
									);
									if (hasDeclaredDeeper || hasMissingDeeper) {
										return;
									}
								}
	...
	if (missingDepsList.length > 0) {
		const isEffect =
			/use(Effect|LayoutEffect|InsertionEffect|ImperativeHandle)/.test(
				reactiveHookName,
			);
	...
	if (unnecessaryDependencies.size > 0) {
		...
		unnecessaryDepsList.forEach((depKey: string): void => {
			...
			if (!isFirst && depTokenIndex > 0) {
				...
				const prevTokenEnd =
					depTokenIndex > 1 ? tokens[depTokenIndex - 1]?.range[1] : 0;

				const whitespaceBefore = sourceCode.text.slice(
					prevTokenEnd,
					removeStart,
				);
	...
	if (duplicateDependencies.size > 0) {
		...
		const normalizedMap = new Map<
			string,
			Array<{ original: string; node: TSESTree.Node }>
		>();
		...
		for (const [depKey, dep] of allDeps) {
			...
			normalizedMap
				.get(normalizedKey)
				?.push({ original: depKey, node: dep.node });
		}
		...
		if (duplicates.length > 0) {
			...
			const duplicateGroups = new Map<
				string,
				Array<(typeof duplicates)[number]>
			>();
			...
			// Report the summary
			const messageId = isSingleDuplicate
				? "duplicateDependency"
				: "duplicateDependencies";
...
export const exhaustiveDepsRule = ESLintUtils.RuleCreator(
	(name: string): string => {
	...
	meta: {
		...
		messages: {
			...
			addDependencies:
				"Add '{{count}}' missing dependencies ('{{dependencies}}')",
			addAllDependencies:
				"Add all '{{count}}' missing dependencies ('{{dependencies}}')",
			addSingleDependency: "Add missing dependency: '{{dependency}}'",
			removeDependencyArray:
				"Remove dependency array to run effect on every render",

			removeDependency:
				"Remove the '{{dependency}}' dependency from the dependency array. ",
	...
	create(
		...
		recordMetric(perfKey, "config", {
			additionalHooks:
				typeof option?.additionalHooks !== "undefined" &&
				option.additionalHooks !== ""
					? "custom"
					: "default",
			experimental_autoDependenciesHooks:
				option?.experimental_autoDependenciesHooks?.length ?? 0,
			requireExplicitEffectDeps: option?.requireExplicitEffectDeps ?? false,
			enableAutoFixForMemoAndCallback:
				option?.enableAutoFixForMemoAndCallback ?? false,
		});
		...
		return {
			...
			[AST_NODE_TYPES.CallExpression](node: TSESTree.CallExpression): void {
				...
				const declaredDependenciesNode:
					...
					maybeNode &&
					!(
						maybeNode.type === AST_NODE_TYPES.Identifier &&
				...
				const isAutoDepsHook =
					option?.experimental_autoDependenciesHooks?.includes(
						"name" in nodeWithoutNamespace ? nodeWithoutNamespace.name : "",
					);

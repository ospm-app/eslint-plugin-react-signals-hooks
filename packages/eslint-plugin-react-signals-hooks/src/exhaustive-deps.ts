  ...
  dependencies.forEach(({ isStable, references }: Dependency, key: string): void => {
    ...
    for (const reference of references) {
      if (reference.writeExpr) {
        ...
        const severity = getSeverity('useEffectEventInDependencyArray', context.options[0]);
...
function visitFunctionWithDependencies(
  ...
  currentRefsInEffectCleanup.forEach(({ reference, dependencyNode }): void => {
    ...
    const severity = getSeverity('staleAssignmentDependency', context.options[0]);
    ...
    if ('elements' in arrayExpression) {
      arrayExpression.elements.forEach((declaredDependencyNode): void => {
        ...
        if (declaredDependencyNode.type === AST_NODE_TYPES.SpreadElement) {
          ...
          const severity = getSeverity('spreadElementInDependencyArray', context.options[0]);
        ...
        try {
          ...
          if (
            ...
            const severity = getSeverity('dependencyWithoutSignal', context.options[0]);
          ...
          if (error instanceof Error && /Unsupported node type/.test(error.message)) {
            if (declaredDependencyNode.type === 'Literal') {
              if (
                ...
                const severity = getSeverity('staleAssignmentLiteral', context.options[0]);
                ...
                const severity = getSeverity('staleAssignmentUnstable', context.options[0]);
              ...
              const severity = getSeverity('staleAssignmentExpression', context.options[0]);

...
class MetricsAggregator {
	...
	public getAggregatedMetrics(): AggregatedMetrics {
		...
		this.metrics.forEach((metric) => {
			...
			aggregated.maxMemoryDelta = Math.max(
				aggregated.maxMemoryDelta,
				memoryDelta,
			);
			aggregated.minMemoryDelta = Math.min(
				aggregated.minMemoryDelta,
				memoryDelta,
			);
			...
			if (metric.operationCounts) {
				Object.entries(metric.operationCounts).forEach(([op, count]) => {
					operationCounts.set(op, (operationCounts.get(op) || 0) + count);
					...
					if (
						...
						count >
							(metric.perfBudget?.maxOperations?.[
								op as keyof typeof PerformanceOperations
							] || 0)
					) {
						exceededOperations.set(op, (exceededOperations.get(op) || 0) + 1);
					}
				});
			}
		...
		aggregated.avgMemoryDelta =
			this.metrics.reduce((sum, m) => sum + (m.memoryDelta || 0), 0) /
		...
		aggregated.commonExceededOperations = Array.from(
			exceededOperations.entries(),
		)
			.sort((a, b) => b[1] - a[1])
			.slice(0, 5)

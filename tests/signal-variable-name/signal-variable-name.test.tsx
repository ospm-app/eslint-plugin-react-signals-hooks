...
export function TestUsePrefix(): JSX.Element {
	useSignals();
	const useSomethingSignal = signal(0); // Should be 'somethingSignal'
	const useCounterSignal = signal(10); // Should be 'counterSignal'
	const useDataSignal = computed(() => useSomethingSignal.value * 2); // Should be 'dataSignal'
...
export function TestMissingSignalSuffix(): JSX.Element {
	...
	const computed1 = computed(() => counter.value + 1); // Should be 'computed1Signal'
...
export function TestMultipleIssues(): JSX.Element {
	...
	const useCounter = signal(10); // Should be 'counterSignal'
	const Data = signal("hello"); // Should be 'dataSignal'
	const useComputedValue = computed(() => UseSomething.value + 1); // Should be 'computedValueSignal'
...
export function TestCorrectNaming(): JSX.Element {
	...
	const userDataSignal = signal(null); // Correct
...
export const TestArrowFunction = (): JSX.Element => {
	...
	const useData = signal("test"); // Should be 'dataSignal'

...
export const exhaustiveDepsRule: Rule.RuleModule = {
	...
	create(context: Rule.RuleContext) {
		...
		function reportProblem(problem: Rule.ReportDescriptor) {
			// Check if this error is autofixable
			const hasAutofix = !!(
				problem.fix ||
				(enableDangerousAutofixThisMayCauseInfiniteLoops &&
			...
			// Add autofixable indicator to the message
			if (hasAutofix || hasSuggestions) {
				...
				// Handle both message and messageId cases
				if ("message" in problem && typeof problem.message === "string") {
					(problem as any).message = problem.message + indicator;
				}
			}
		...
		function visitFunctionWithDependencies(
			...
			function isStableKnownHookValue(resolved: Scope.Variable): boolean {
				...
				if (
					(isSignalIdentifier(callee) || isSignalVariable(id)) &&
					id.type === "Identifier"
				) {
			...
			function isOnlyAssignmentReference(reference: Scope.Reference): boolean {
				...
				if (
					...
					// Check if it's a direct assignment: signal.value = something
					...
					const isSignalReference =
						...
						(reference.identifier.name.endsWith("Signal") ||
							...
							(reference.resolved &&
								...
								reference.resolved.name.endsWith("Signal")));
            ...
            if (propertyName !== "current") {
              ...
              objectPropertyAccesses.get(objectName)!.add(fullPath);
            }
          }
        ...
        // Don't continue - let it fall through to process the function reference as a dependency
              ...
              if (
                ...
                if (referenceNode.parent.parent.computed && referenceNode.parent.parent.property) {
                  ...
                  if (propertyName) {
                    ...
                    if (propertyRef && propertyRef.resolved) {
                ...
                dependencyNode = referenceNode.parent;
					...
					// @ts-ignore Check if this is a computed assignment-only reference
					...
					if (dependencies.has(dependency)) {
						...
						if (!isAssignment && !isComputedAssignmentOnly) {
							dependencies.get(dependency)!.hasReads = true;
						}
					} else {
						...
						if (isImportedSignal || isSignalValueAccessBool) {
							if (isImportedSignal && !isSignalValueAccessBool) {
								const signalName = dependency.includes(".")
									? dependency.split(".")[0]
									: dependency;
					...
					(declaredDependencyNode) => {
						...
						try {
							...
							const hasParentheses =
								dependencyText.includes("(") && dependencyText.includes(")");
			...
			function reportProblem(descriptor: Rule.ReportDescriptor) {
				// Check if this error is autofixable (same logic as main reportProblem)
				const hasAutofix = !!(
					descriptor.fix ||
					(enableDangerousAutofixThisMayCauseInfiniteLoops &&
				...
				const hasSuggestions = !!(
					Array.isArray(descriptor.suggest) && descriptor.suggest.length > 0
				);
				...
				if (
					...
					(descriptor as any).message = descriptor.message + indicator;
				}

				if (
					"message" in descriptor &&
					// @ts-ignore
					descriptor.message?.includes("unnecessary dependency")
				) {
					// @ts-ignore
					const match = descriptor.message?.match(
						/unnecessary dependency: '([^']+)'/i,
					);
					...
					if (key.includes("Signal") && key.includes(".")) {
						...
						if (parts.length > 2 && parts[1] === "value") {
							...
							const isAssignmentOnly =
								dependency && dependency.hasReads === false;
			...
			const missingMessage = getWarningMessage(
				missingDependencies,
				"a",
				"missing",
				"include",
			...
			const duplicateMessage = getWarningMessage(
				duplicateDependencies,
				"a",
				"duplicate",
				"omit",
...
function collectRecommendations({
	...
	dependencies.forEach((_dep, key): void => {
		if (key.endsWith(".value")) {
			...
			if (!dependencies.has(valueKey) && !hasDeepPropertyChains) {
				const hasDeepChains = Array.from(importedSignals).some(
					(sig: string): boolean => {
						return (
							sig.startsWith(`${key}.value[`) &&
							sig.includes(".", sig.indexOf(".value[") + 7)
						);
					},
				);
	...
	importedSignals.forEach((signal) => {
		...
		const isDeclared =
			declaredDependencies.some(({ key }) => key === signal) ||
		...
		if (!key.includes(".") && !key.endsWith("Signal")) {
			...
			if (hasDeepPropertyUsage) {
				...
				if (hasPropertyAlsoDeclared) {
					...
					// Only mark as redundant if it's not directly used
					if (!isDirectlyUsed) {
						redundantDependencies.add(key);
						satisfyingDependencies.delete(key);
					}
	...
	if (node.type === "MemberExpression") {
		...
		if (node.property.type === "Identifier") {
			...
			try {
				...
				// biome-ignore format: because
				// @ts-ignore - TypeScript AST node types not in estree

...
export function TestSignalValueMapComplex(): JSX.Element {
  ...
  return (
    <div>
      {usersSignal.value.map((user, index): JSX.Element => {
        return (
...
export function TestDirectSignalMap(): JSX.Element {
  ...
  return (
    <div>
      {numbersSignal.value.map((num: number, index: number): JSX.Element => {
        // biome-ignore lint/suspicious/noArrayIndexKey: not relevant
        return <span key={index}>{num * 2}</span>;
      })}
    </div>
  );
}
...
export function TestSignalMapArrowFunction(): JSX.Element {
  ...
  return (
    <div>
      {colorsSignal.value.map((color: string): JSX.Element => {
        return (
...
export function TestSignalMapFunctionExpression(): JSX.Element {
  ...
  return (
    <ul>
      {tasksSignal.value.map(function (
  ...
  return (
    <div>
      {categoriesSignal.value.map(
        (category: { name: string; items: string[] }, catIndex: number): JSX.Element => {
          return (
            // biome-ignore lint/suspicious/noArrayIndexKey: not relevant
            <div key={catIndex}>
              <h3>{category.name}</h3>
              ...
              <ul>
                {category.items.map((item: string, itemIndex: number): JSX.Element => {
                  // biome-ignore lint/suspicious/noArrayIndexKey: not relevant
...
export function TestRegularArrayMap(): JSX.Element {
  ...
  return (
    <ul>
      {regularArray.map((item: string, index: number): JSX.Element => {
        // biome-ignore lint/suspicious/noArrayIndexKey: not relevant
        return <li key={index}>{item}</li>;
      })}
    </ul>
...
export const TestArrowFunctionSignalMap = (): JSX.Element => {
  ...
  return (
    <div>
      {dataSignal.value.map((value: number, index: number): JSX.Element => {
        return (
          // biome-ignore lint/suspicious/noArrayIndexKey: not relevant
  ...
  return (
    <div>
      {numbersSignal.value
        .filter((num) => num % 2 === 0)
        .map(
          (evenNum: number, index: number): JSX.Element => (
            // biome-ignore lint/suspicious/noArrayIndexKey: not relevant
...
export function TestMultipleSignalMaps(): JSX.Element {
  ...
  return (
    <div>
      ...
      <ul>
        {fruitsSignal.value.map((fruit: string, index: number): JSX.Element => {
          // biome-ignore lint/suspicious/noArrayIndexKey: not relevant
      ...
      <ul>
        {vegetablesSignal.value.map((vegetable: string, index: number): JSX.Element => {
          // biome-ignore lint/suspicious/noArrayIndexKey: not relevant
          return <li key={index}>{vegetable}</li>;
        })}
      </ul>
    </div>
  );
